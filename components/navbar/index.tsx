        ...
        <header className="bg-[#146eb4] text-white py-3 md:px-14 px-5 z-10 relative">
            <div className="flex justify-between items-center">
                ...
                <nav className="md:flex gap-10 hidden">
                    {navLinks.map(link => (
                        <Link key={link.label} className="hover:bg-[#ffffff22] px-2 py-1 text-center rounded-lg transition-all" href={link.href}>
                            {link.label}
                        </Link>
                    ))}
                </nav>
            ...
            <div className={`fixed text-black border h-full z-40 bg-white top-0 right-0 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'} w-full md:w-[24rem]`}>
                ...
                <nav className="px-10 mt-10 flex gap-5 flex-col">
                    {navLinks.map(link => (
                        <Link onClick={() => setIsOpen(false)} key={link.label} href={link.href}>
                            {link.label}
                        </Link>
                    ))}
                </nav>
