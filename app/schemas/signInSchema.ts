...
export const signInSchema = z.object({
    name: z.string().min(4, "Name must be at least 4 characters").max(100, "Name can't be more than 100 characters"),
    email: z.string()
        .email("This is not a valid email")
        .regex(/^[a-zA-Z0-9._%+-]+@(?:[a-zA-Z0-9-]+\.)?pdpu\.ac\.in$/, "Email must be a college email"),
    phone: z.string().regex(/^([+]?[\s0-9]+)?(\d{3}|[(]?[0-9]+[)])?([-]?[\s]?[0-9])+$/, "Invalid phone number"),
